import React from 'react'
import { CtuLogo } from '@/Components/CtuLogo'
import { FiGrid } from "react-icons/fi";
import { GoPeople } from "react-icons/go";
import { BiCoinStack } from "react-icons/bi";
import { PageProps } from '@/types';
import { Link } from 'lucide-react';
import NavLink from './NavLink';
import { usePage } from '@inertiajs/react';
type Stats = {
  pendingApprovals: number;
};

type PagePropsWithStats = PageProps & {
  stats: Stats;
};
export default React.memo(function Sidebar({ auth }: PageProps) {
  const { stats } = usePage<PagePropsWithStats>().props;

  const linkClass = (isActive: boolean) =>
    `inline-flex items-center px-3 py-2 rounded-lg text-sm font-medium transition duration-300 ease-in-out  ${isActive ? 'bg-[#006654] hover:bg-[#006654] text-white' : 'text-white hover:bg-[#006654]'
    }`;

  return (
    <div className="hidden md:block">

      <div className='fixed top-0 left-2 rounded-2xl w-[5em] max-h-screen h-[93%] mx-5 my-6 bg-green-100 bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-50 flex flex-col justify-between'>
        <div className="p-2">
          <a href='/' className='flex items-center justify-center'>
            <CtuLogo className='p-1 w-20 h-15' />
          </a>

          <div className="flex flex-col items-center justify-center gap-5 h-[500px]">
            {auth.user.role === 'Admin' ? (
              <>
                <NavLink title="Dashboard" href={route('admin.dashboard')} className={linkClass(route().current('admin.dashboard'))}>
                  <FiGrid className='text-2xl' />
                </NavLink>
                <NavLink
                  title="Employees"
                  href={route('admin.employee')}
                  className={linkClass(route().current('admin.employee'))}
                >
                  <div className="relative">
                    <GoPeople className="text-2xl" />

                    {stats?.pendingApprovals > 0 && (
                      <span className="absolute -top-2 -right-2 text-[10px] bg-red-500 text-white px-2 rounded-full">
                        {stats.pendingApprovals}
                      </span>
                    )}
                  </div>
                </NavLink>
                <NavLink title="Payroll" href={route('admin.payroll')} className={linkClass(route().current('admin.payroll'))}>
                  <img
                    src="data:image/png;base64," alt="external-Payroll-budget-smashingstocks-mixed-smashing-stocks" // your full base64 here
                    style={{ filter: 'invert(1) brightness(2)' }}
                    className="w-6 h-6"
                  />
                </NavLink>
                <NavLink title="Payroll Summary" href={route('admin.payroll.summary')} className={linkClass(route().current('admin.payroll.summary'))}>
                  <img
                    src="data:image/png;base64," alt="external-Payroll-audit-smashingstocks-mixed-smashing-stocks"
                    className='w-6 h-6'
                  />
                </NavLink>
                <NavLink title="HR Configuration" href={route('admin.department')} className={linkClass(route().current('admin.department'))}>
                  <img
                    className="w-6 h-6 "
                    src="data:image/png;base64," alt="organization-chart-people"
                  />
                </NavLink>
              </>
            ) : (
              <div className="p-2 ">


                <div className="flex flex-col items-center  justify-center gap-5 h-[500px]">
                  <NavLink title="Dashboard" href={route('employee.dashboard')} className={linkClass(route().current('employee.dashboard'))}>
                    <FiGrid className='text-2xl text-white' />
                  </NavLink>
                  <NavLink title="Payslips" href={route('employee.payslip.summary')} className={linkClass(route().current('employee.payslip.summary'))} >
                    <BiCoinStack className='font-black text-2xl text-white' />
                  </NavLink>
                </div>
              </div>
            )
            }

          </div>
        </div>
      </div>
    </div>
  )
})