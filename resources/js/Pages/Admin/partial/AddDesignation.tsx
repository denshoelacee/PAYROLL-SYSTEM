...
export default function AddDesignation({jobTitleList}:Props) {
    ...
    const {data, setData, post, reset } = useForm<any>({
        designation:'',
        checker:'toDesignation'
    });

    const designationRows = jobTitleList
    .filter(dep => dep.designation && dep.designation.toUpperCase() !== 'NULL')
    .map((dep) => ({
        id: dep.id,
        designation: dep.designation
    }));
    
        const handleSubmit = (actionType: 'toDesignation'): FormEventHandler => {
            return (e) => {
                e.preventDefault(); 
        ...
        const HandleEditSubmit =(actionType: 'toDesignation'): FormEventHandler => {
            return (e) => {
            e.preventDefault();
                setData('checker', actionType); 
        ...
        const deleteSubmit = (actionType: 'toDesignation'): FormEventHandler => {
            return (e) => {
                e.preventDefault();
                setData('checker', actionType);
        ...
        {field: 'actions', headerName: 'Actions', flex:1,headerAlign:'center',sortable:false,renderCell: (params) => (
            <div className="flex gap-2 pt-1 items-center justify-center">
                <div onClick={() => {
                            ...
                            setData('designation', params.row.designation); 
                            setEditModal(true);
            ...
            <Modal show={showModal} onClose={() => setShowModal(false)} maxWidth="sm">
                ...
                <div className="p-6">
                    ...
                    <div className="flex justify-end gap-2 mt-3">
                        ...
                        <PrimaryButton onClick={handleSubmit('toDesignation')}>Submit</PrimaryButton>
                    </div>
                </div>
                </form>
            </Modal>
            <Modal show={editModal} onClose={() => setEditModal(false)} maxWidth="sm">
                ...
                <div className="p-6">
                    ...
                    <div className="flex justify-end gap-2 mt-3">
                        ...
                        <PrimaryButton onClick={HandleEditSubmit('toDesignation')}>Submit</PrimaryButton>
                    </div>
                </div>
                </form>
            </Modal>
            <Modal show={deleteModal} onClose={() => setDeleteModal(false)} maxWidth='sm' >
                    ...
                    <p className="text-white mb-4">
                        ...
                        <div className="flex justify-evenly gap-3 py-3">
                            <PrimaryButton className="py-2"onClick={deleteSubmit('toDesignation')}>
                            Confirm
                            </PrimaryButton>
