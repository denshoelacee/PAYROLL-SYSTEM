...
export default function Login({
  ...
  // ✅ Debounced setter to limit unnecessary updates
  const debouncedSetEmployeeId = useCallback(
  ...
  const inputHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (/^[0-9]*$/.test(value) && value !== data.employee_id) {
      debouncedSetEmployeeId(value);
    }
  };
  ...
  const submit: FormEventHandler = (e) => {
    e.preventDefault();
    post(route('login'), {
      preserveState: true, // ✅ Prevent unnecessary DB reloads
    });
  };
    ...
    <LoginLayout>
      ...
      <form onSubmit={submit}>
        <div className="mt-1 sm:mt-2 lg:mt-2">
          {message?.information && <InfoMessage className="my-2" info={message.information} />}
          {message?.error && <ErrorMessage className="my-2" error={message.error} />}
        ...
        <div className="flex justify-center text-white gap-1">
          ...
          <Link href={route('register')} className="text-white text-md">
